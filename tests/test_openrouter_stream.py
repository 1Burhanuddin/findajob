...
# ---------------------------------------------------------------------------
# Test 11 (#743): is_cancelled callback aborts the stream early
# ---------------------------------------------------------------------------


def test_is_cancelled_aborts_without_terminal_chunk(monkeypatch, tmp_path):
    """is_cancelled() returning True mid-stream halts iteration before [DONE].

    Asserts the cancellation contract: NO StreamFinish yielded, resp.close()
    invoked via the existing try/finally cleanup. Caller distinguishes
    cancellation from natural finish by absence of a terminal chunk.
    """
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or-v1-test")
    roles = _setup_role(tmp_path)

    # Three delta chunks; cancellation should fire before the third's content
    # is even decoded.
    payloads = [
        _delta_chunk("first"),
        _delta_chunk("second"),
    ...
    # Cancel after the second SSE line read returns True (lines come in pairs:
    # data line + blank-separator line, so call_count threshold fires partway).
    call_count = {"n": 0}

    def _cancel_after_two_calls() -> bool:
        call_count["n"] += 1
    ...
    with patch("findajob.llm.openrouter.urllib.request.urlopen", return_value=fake_resp):
        chunks = list(
            complete_stream(
                ...
                prompt="test",
                roles_dir=roles,
                is_cancelled=_cancel_after_two_calls,
            )
        )

    # No terminal StreamFinish — that's the cancellation signal to callers.
    assert not any(c["type"] == "finish" for c in chunks), "cancellation must NOT yield a terminal StreamFinish"
    # No StreamError either — cancellation is normal, not failure.
    assert not any(c["type"] == "error" for c in chunks)
    # Cleanup ran via the try/finally — same path as GeneratorExit.
    assert fake_resp.close_called


def test_is_cancelled_none_means_no_check(monkeypatch, tmp_path):
    """is_cancelled=None (default) preserves pre-#743 behavior — natural finish."""
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or-v1-test")
    roles = _setup_role(tmp_path)
    ...
    # Explicit None — same as omitting the kwarg.
    with patch("findajob.llm.openrouter.urllib.request.urlopen", return_value=fake_resp):
        chunks = list(
            complete_stream(
                role="test_role",
    ...
    # Natural finish — terminal StreamFinish present.
    finish = [c for c in chunks if c["type"] == "finish"]
    assert len(finish) == 1
    assert finish[0]["text"] == "natural finish"
