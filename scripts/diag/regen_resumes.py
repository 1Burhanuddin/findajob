...
from paths import BASE, AICHAT, PANDOC
...
def main():
    ...
    for folder_name in folders:
        ...
        if os.path.exists(v2_path):
            print(f"  SKIP — tailored_resume_DRAFT_v2.md already exists")
            continue
        ...
        if not jd_text:
            print(f"  WARNING — no JD found, proceeding with title/company from folder name only")
        ...
        try:
            ...
            print(f"  ERROR — aichat-ng timed out")
            continue
        ...
        print(f"  DONE → tailored_resume_DRAFT_v2.md + .docx")
