...
def check_core_competencies(sections):
    ...
    if not found:
        violations.append(violation('MED', 'core_competencies_missing',
                                    'No "## Core Competencies" section found'))
    return violations
...
def check_summary(sections):
    ...
    if sentences < SUMMARY_MIN_SENTENCES:
        violations.append(violation('LOW', 'summary_too_short',
                                    f'Summary has ~{sentences} sentence(s); minimum is {SUMMARY_MIN_SENTENCES}'))
    elif sentences > SUMMARY_MAX_SENTENCES:
        violations.append(violation('LOW', 'summary_too_long',
                                    f'Summary has ~{sentences} sentences; maximum is {SUMMARY_MAX_SENTENCES}'))
    return violations
...
def check_role_block(block, is_primary):
    ...
    # Count bullets
    bullet_lines = [(n, l) for n, l in role_lines if re.match(r'^- ', l)]
    bullet_count = len(bullet_lines)
    max_bullets = PRIMARY_ROLE_MAX_BULLETS if is_primary else OTHER_ROLE_MAX_BULLETS

    if bullet_count > max_bullets:
        sev = 'HIGH' if is_primary else 'MED'
        violations.append(violation(sev, 'excess_bullets',
                                    f'"{role_label}" has {bullet_count} bullets (limit: {max_bullets})',
                                    header_line))

    if bullet_count < MIN_BULLETS_PER_ROLE:
        violations.append(violation('MED', 'too_few_bullets',
    ...
    non_bullet_non_blank = [(n, l) for n, l in role_lines
    ...
    for lineno, line in non_bullet_non_blank:
        ...
        if re.match(r'^\*[^*]', stripped):
            violations.append(violation('HIGH', 'italic_prose',
                                        f'Italic context sentence in "{role_label}": {stripped[:80]}',
                                        lineno))
        # Check: bold group label (starts with ** followed by capital or common label)
        elif re.match(r'^\*\*[A-Z]', stripped):
            violations.append(violation('HIGH', 'bold_group_label',
                                        f'Bold thematic label in "{role_label}": {stripped[:80]}',
                                        lineno))
        # Check: bold/italic subtitle (starts with _ or __ not a bullet)
        elif re.match(r'^_{1,2}[A-Z]', stripped):
            violations.append(violation('HIGH', 'italic_subtitle',
                                        f'Italic subtitle in "{role_label}": {stripped[:80]}',
                                        lineno))
        # Check: plain prose paragraph (non-empty, none of the above, not a bullet)
        elif stripped and not re.match(r'^[-*_#|]', stripped):
            violations.append(violation('MED', 'prose_paragraph',
                                        f'Prose paragraph in "{role_label}": {stripped[:80]}',
                                        lineno))
...
def check_length(lines):
    ...
    if pages > MAX_ESTIMATED_PAGES:
        violations.append(violation('MED', 'too_long',
                                    f'Estimated length {pages:.1f} pages (max {MAX_ESTIMATED_PAGES})'))
    return violations
...
def check_violations(filepath):
    ...
    if role_blocks:
        bullet_counts = [
            sum(1 for _, l in b['lines'] if re.match(r'^- ', l))
...
def main():
    ...
    if use_json:
        ...
        if len(targets) > 1:
            ...
            total_high = sum(1 for v in all_results.values()
                             for viol in v if viol['severity'] == 'HIGH')
