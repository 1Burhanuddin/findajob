...
_TITLE_KEYWORDS = [
    'data center', 'datacenter', 'operations', 'manager', 'technician',
    'engineer', 'senior', 'director', 'lead', 'infrastructure', 'systems',
    'hardware', 'technical', 'program', 'manufacturing', 'quality',
    'deployment', 'site', 'npi', 'ai', 'forward deployed',
]
...
def analyze(conn):
    ...
    for kw in _TITLE_KEYWORDS:
        ...
        keyword_signals.append({
            'keyword': kw, 'applied_pct': a_pct, 'rejected_pct': r_pct, 'ratio': ratio,
            'applied_n': a_count, 'rejected_n': r_count,
        })
    ...
    for t in fp_titles:
        ...
        for w in words:
            if w not in ('and', 'the', 'for', 'with', 'data', 'center', 'senior',
                         'lead', 'manager', 'director', 'engineer', 'operations'):
                title_word_freq[w] += 1
...
def format_report(data):
    ...
    if bad_kws:
        ...
        for kw in bad_kws[:8]:
            lines.append(f"    - \"{kw['keyword']}\"  ({kw['rejected_n']} rejected, {kw['rejected_pct']}% rate, ratio={kw['ratio']})")
    ...
    if notify_flag:
        ...
        subprocess.run(
            ['curl', '-s', '-X', 'POST', f'https://ntfy.sh/{topic}',
             '-H', 'Title: JSP Feedback Analysis',
             '-H', 'Tags: magnifying_glass',
