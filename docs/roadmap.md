...
GA shipped 2026-05-04 (Decision 20); Multi-Tenancy Foundations shipped 2026-05-06 (Decision 21, ahead of due 2026-05-18). The active phase is a parallel close-out of four concurrent open milestones over a ~30-day window before pivoting to Open-Source Launch Readiness.

**Active goal:** ship the four open milestones to their deliverable sentences (see [Post-GA horizon](#post-ga-horizon) below for status of each), then pivot to OS Launch Readiness (#377).
...
**Milestone names are now deliverable-anchored, not version-numbered.** Chronological release order is GA → Multi-Tenancy Foundations → OpenRouter Native Migration → Cost + Credentials Hardening → Funnel + Triage UX → Stats Dashboards → Ops Hardening → Tuning Loop. Funnel + Triage UX ships before Stats Dashboards / Ops Hardening / Tuning Loop per Decision 14's "funnel-first" framing. Semver release tags (v0.9.x, v0.10.x, v1.0.0, …) float on the release-process schedule independently of milestone names.
...
- **[Ops Hardening](https://github.com/brockamer/findajob/milestone/8)** (due 2026-05-29) — "Fresh-install smoke is CI-gated, pipeline.jsonl rotates, DB schema migrates cleanly, and folder/DB drift is detectable on demand." Can ship in parallel with Stats Dashboards / Tuning Loop; date is outside-in.
