...
GA shipped 2026-05-04 (Decision 20); Multi-Tenancy Foundations shipped 2026-05-06 (Decision 21, ahead of due 2026-05-18); Cost + Credentials Hardening and OpenRouter Native Migration shipped between 2026-05-06 and 2026-05-09 (Decision 22). The active phase is a parallel close-out of four concurrent open milestones, with Open-Source Launch Readiness milestoned in advance as the named post-arc horizon (Decision 22).

**Active goal:** ship the four open milestones to their deliverable sentences (see [Post-GA horizon](#post-ga-horizon) below for status of each), then ramp into OS Launch Readiness (#377, m11).

The most strategically-anchoring milestone of the four is **Funnel + Triage UX** — its deliverable is closest to the operator's daily apply-gate goal. The other three (Stats Dashboards, Tuning Loop, Ops Hardening) ship in parallel as visualization, tuning substrate, and ops reliability.
...
**Milestone names are now deliverable-anchored, not version-numbered.** Chronological release order is GA → Multi-Tenancy Foundations → OpenRouter Native Migration → Cost + Credentials Hardening → Funnel + Triage UX → Stats Dashboards → Ops Hardening → Tuning Loop → Open-Source Launch Readiness. Funnel + Triage UX ships before Stats Dashboards / Ops Hardening / Tuning Loop per Decision 14's "funnel-first" framing. Semver release tags (v0.9.x, v0.10.x, v1.0.0, …) float on the release-process schedule independently of milestone names.
...
- **[Stats Dashboards](https://github.com/brockamer/findajob/milestone/13)** (due 2026-05-27) — "The /stats/* pages render score distribution, rejection breakdown, application throughput, and feedback-effectiveness charts from live data." Visualization sibling of Tuning Loop.
...
- **[Open-Source Launch Readiness](https://github.com/brockamer/findajob/milestone/11)** (due 2026-06-29) — "An external job-seeker who finds the repo via the OS launch can clone, follow `docs/setup/install-docker.md` end-to-end, complete onboarding, and reach a daily-utility state without contacting the operator." Promoted in advance of Ops Hardening shipping (Decision 22); preparation work runs in parallel with the active close-out arc rather than waiting for it. Anchored by epic #377.
...
**Open-source launch readiness** — [#377](https://github.com/brockamer/findajob/issues/377). *Promoted to active milestone (m11, due 2026-06-29) on 2026-05-06 ahead of Ops Hardening shipping (Decision 22) — preparation work runs in parallel with the active close-out arc rather than waiting for it.* Public-repo polish: domain-neutral README, external user install guide with zero operator intervention, `GENERALIZATION.md` complete, CI green for external contributors, LICENSE / CONTRIBUTING / CODE_OF_CONDUCT.
